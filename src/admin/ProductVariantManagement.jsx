    ...
    try {
      ...
      for (const variant of variantsToCreate) {
        try {
          ...
          // ✅ TIDAK ADA gambarUrl - backend tidak support
          const payload = {
            sku,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-pink-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="bg-white rounded-3xl shadow-sm border border-pink-100 p-6">
          <div className="flex items-center gap-4">
            <button
              ...
              className="p-3 hover:bg-pink-50 rounded-xl transition-all text-gray-600 hover:text-[#cb5094]"
            >
          ...
          {selectingImageForColor && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-5">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-pink-100 p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-3">{product.nama}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-pink-50 rounded-xl p-3">
              <p className="text-xs text-gray-600 mb-1">Kategori</p>
              <p className="font-semibold text-gray-800 text-sm">{product.category?.nama || '-'}</p>
            </div>
            <div className="bg-pink-50 rounded-xl p-3">
              <p className="text-xs text-gray-600 mb-1">Harga Dasar</p>
              <p className="font-semibold text-[#cb5094] text-sm">{formatPrice(product.hargaDasar)}</p>
            </div>
            <div className="bg-pink-50 rounded-xl p-3">
              <p className="text-xs text-gray-600 mb-1">Total Varian</p>
              <p className="font-semibold text-gray-800 text-sm">{safeVariants.length} varian</p>
            </div>
            <div className="bg-pink-50 rounded-xl p-3">
              <p className="text-xs text-gray-600 mb-1">Total Stok</p>
              <p className="font-semibold text-gray-800 text-sm">{totalStock} pcs</p>
            </div>
          </div>
        </div>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full my-8 max-h-[90vh] flex flex-col">
              <div className="bg-gradient-to-r from-[#cb5094] to-[#e570b3] px-5 py-4 flex justify-between items-center rounded-t-3xl">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 px-4">
              ...
              <p className="text-gray-600 mb-6 text-sm">
                Klik tombol "Tambah Varian" untuk membuat varian produk
              </p>
              <button
                onClick={() => setShowForm(true)}
                className="bg-[#cb5094] text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-[#b54684] inline-flex items-center gap-2"
              >
                <Plus className="w-4 h-4" />
                Tambah Varian
              </button>
                ...
                <tbody className="divide-y divide-gray-100">
                  {safeVariants.map((variant) => (
                    <tr key={variant.id} className="hover:bg-pink-50/50 transition-colors">
                      {/* ✅ Gambar dari localStorage berdasarkan warna */}
