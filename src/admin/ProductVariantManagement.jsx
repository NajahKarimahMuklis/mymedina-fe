...
function ProductVariantManagement() {
  ...
  // SYNC HARGA BY COLOR
  // ===================================================================
  const handleSyncPriceByColor = async (colorToSync, newPrice) => {
    try {
      const variantsWithSameColor = variants.filter(
        (v) =>
          v.warna.toLowerCase() === colorToSync.toLowerCase() &&
      ...
      for (const variant of variantsWithSameColor) {
        try {
          ...
          console.error(`Gagal sync harga untuk varian ${variant.sku}:`, error);
          failCount++;
        }
      }
      if (successCount > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingVariant) {
      const variantsWithSameColor = variants.filter(
        (v) =>
          v.warna.toLowerCase() === formData.warna.toLowerCase() &&
          v.id !== editingVariant.id
      );
      ...
      if (
        isPriceChanged &&
        !isImageChanged &&
        variantsWithSameColor.length > 0
      ) {
        ...
        setShowPriceSyncConfirmation(true);
        return;
      }
    }

    await saveVariant(newImage, newPrice, false, false);
  };
  ...
  const saveVariant = async (
    ...
    try {
      if (editingVariant) {
        ...
        if (shouldSyncPrice) {
          await handleSyncPriceByColor(formData.warna, newPrice);
        }
      } else {
        await variantAPI.create(productId, payload);
        toast.dismiss(loadingToast);
  ...
  const handleConfirmSync = async (shouldSync) => {
    ...
    if (pendingImageUpdate) {
      await saveVariant(
        pendingImageUpdate.newImage,
        pendingImageUpdate.newPrice,
        shouldSync,
        false
      );
  ...
  const handleConfirmPriceSync = async (shouldSync) => {
    setShowPriceSyncConfirmation(false);

    if (pendingPriceUpdate) {
      await saveVariant(
        pendingPriceUpdate.newImage,
  ...
  const handleEdit = (variant) => {
    ...
    setFormData({
      ...
      hargaOverride:
        variant.hargaOverride !== null ? variant.hargaOverride.toString() : "",
      aktif: variant.aktif,
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-pink-50 rounded-xl p-4 text-center">
            ...
            <p className="font-semibold text-[#cb5094]">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl shadow-sm border border-pink-100 p-5 text-center">
            <div className="text-3xl font-bold text-[#cb5094] mb-1">
              {uniqueSizes.length}
            </div>
                  ...
                  >
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <code className="text-xs font-mono bg-gray-50 px-2 py-1 rounded block mb-2 truncate">
                          {variant.sku}
                        </code>
                        ...
                        <p className="font-bold text-[#cb5094] text-lg">
                          {formatPrice(
                            variant.hargaOverride || product.hargaDasar
                          )}
                        </p>
                        <p className="text-sm text-gray-600 mt-1">
                          Stok:{" "}
                          <span className="font-bold">{variant.stok} pcs</span>
                        </p>
                        {variant.aktif ? (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-center gap-3">
                              ...
                              <button
                                onClick={() =>
                                  handleDelete(variant.id, variant.sku)
                                }
            ...
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Ditemukan {pendingImageUpdate.affectedCount} varian lain
                    dengan warna "{pendingImageUpdate.colorToSync}"
                  </p>
                </div>
              </div>
              <div className="bg-blue-50 rounded-xl p-4 mb-6">
                <p className="text-sm text-gray-700 leading-relaxed">
                  ...
                  <strong className="text-[#cb5094]">
                    otomatis mengupdate foto
                  </strong>{" "}
                  untuk semua varian dengan warna{" "}
                  <strong>"{pendingImageUpdate.colorToSync}"</strong>?
                </p>
                <p className="text-xs text-gray-600 mt-2">
                  ✨ Ini akan membuat semua varian dengan warna yang sama
        ...
        {/* Sync Confirmation Modal - HARGA */}
        {showPriceSyncConfirmation && pendingPriceUpdate && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[80] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Ditemukan {pendingPriceUpdate.affectedCount} varian lain
                    dengan warna "{pendingPriceUpdate.colorToSync}"
                  </p>
                </div>
              </div>
              <div className="bg-green-50 rounded-xl p-4 mb-6">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Apakah Anda ingin{" "}
                  <strong className="text-[#cb5094]">
                    otomatis mengupdate harga
                  </strong>{" "}
                  menjadi{" "}
                  <strong className="text-green-600">
                    {pendingPriceUpdate.newPrice
                      ? formatPrice(pendingPriceUpdate.newPrice)
                      : "Default"}
                  </strong>{" "}
                  untuk semua varian dengan warna{" "}
                  <strong>"{pendingPriceUpdate.colorToSync}"</strong>?
                </p>
                <p className="text-xs text-gray-600 mt-2">
                  💰 Semua ukuran dengan warna yang sama akan memiliki harga
                  yang sama
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => handleConfirmPriceSync(false)}
                  className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-xl font-bold hover:bg-gray-200 transition-all"
                >
                  Tidak
                </button>
            ...
            <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">
                  Pilih Gambar Varian{" "}
                  {currentColorForPicker && ` untuk ${currentColorForPicker}`}
                </h3>
                <button
                  onClick={() => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative rounded-xl overflow-hidden border-4 transition-all hover:scale-105 ${
                          isSelected
                            ? "border-[#cb5094] ring-4 ring-[#cb5094]/50"
                            : "border-gray-300"
                        }`}
                      >
                        <img
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                hargaOverride: e.target.value,
                              })
                            }
                            min="0"
                            step="1000"
                            placeholder={`Default: ${formatPrice(
                              product.hargaDasar
                            )}`}
                          ...
                          {sizes.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-4">
                              {sizes.map((size) => (
                                <span
                                  key={size}
                                  className="bg-white border border-blue-500 text-blue-700 px-4 py-2 rounded-full font-medium flex items-center gap-2"
                                >
                                  {size}
                                  <button
                          ...
                          <div className="flex justify-between items-center mb-6">
                            <div>
                              <h4 className="text-xl font-bold text-gray-800">
                                Atur Gambar per Warna
                              </h4>
                              <p className="text-sm text-gray-600 mt-1">
                                Pilih sekali per warna → otomatis berlaku untuk
                                semua ukuran
                              </p>
                            </div>
                            <button
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                            {colors.map((color) => {
                              ...
                              return (
                                <div
                                  key={color}
                                  className="bg-white rounded-2xl p-5 border-2 border-gray-200 shadow-sm text-center"
                                >
                                  <h5 className="font-bold text-lg text-[#cb5094] mb-4">
                                    {color}
                                  </h5>
                                  {currentImage ? (
                                    <img
                          ...
                          <div>
                            ...
                            <div className="max-h-80 overflow-y-auto space-y-3">
                              {variantsToCreate.map((v) => (
                                <div
                                  key={v.id}
                                  className="bg-white rounded-lg p-4 border flex items-center justify-between shadow-sm"
                                >
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    <input
                                      ...
                                      value={v.stok}
                                      onChange={(e) =>
                                        handleStockChange(v.id, e.target.value)
                                      }
