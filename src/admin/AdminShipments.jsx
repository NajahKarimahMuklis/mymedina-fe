...
function AdminShipments() {
  ...
  const fetchOrderForCreate = async (orderId) => {
    try {
      ...
      showNotification(
        "error",
        err.response?.data?.message || "Gagal memuat data order"
      );
      setSearchParams({});
    }
  };
      ...
      for (const order of orders) {
        try {
          ...
          if (err.response?.status !== 404) {
            console.warn(
              `Error fetching shipment for order ${order.id}:`,
              err.message
            );
          }
          continue;
  ...
  const filteredShipments = shipments.filter((s) => {
    ...
    if (
      filterKurir !== "all" &&
      s.kurir?.toLowerCase() !== filterKurir.toLowerCase()
    )
      return false;
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      if (
        !s.nomorResi?.toLowerCase().includes(query) &&
  ...
  const handleCreateShipment = async (e) => {
    ...
    try {
      ...
      showNotification(
        "error",
        err.response?.data?.message || err.message || "Gagal membuat pengiriman"
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleSyncTracking = async (shipment) => {
    try {
      ...
      showNotification(
        "error",
        err.response?.data?.message || "Gagal sync tracking"
      );
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm lg:text-base">
                  Buat, pantau, dan update pengiriman pesanan
                </p>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-pink-100 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Pengiriman akan muncul setelah dibuat dari halaman Pesanan
            </p>
            <button
              onClick={() => (window.location.href = "/admin/orders")}
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-bold text-gray-900">
                          {shipment.nomorResi || "Belum ada resi"}
                        </h3>
                        <p className="text-sm text-gray-500 mt-1">
                          Order: {shipment.order?.nomorOrder}
                        </p>
                        <p className="text-sm text-gray-600">
                          {shipment.order?.namaPenerima}
                        </p>
                      </div>
                      <div
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-center">
                            <span
                              className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold ${statusConfig.color}`}
                            >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-center gap-3">
                              ...
                              {shipment.biteshipOrderId && (
                                <button
                                  onClick={() =>
                                    handleSyncTracking(shipment.id)
                                  }
                                  className="p-2 text-[#cb5094] hover:bg-pink-100 rounded-lg transition-all"
                                  title="Sync Tracking"
                                >
              ...
              <form onSubmit={handleCreateShipment} className="p-6 space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm({ ...createForm, kurir: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCreateForm({ ...createForm, layanan: e.target.value })
                    }
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#cb5094] focus:outline-none"
                  />
                </div>
              ...
              <div className="sticky top-0 bg-gradient-to-r from-[#cb5094] to-[#e570b3] text-white px-8 py-6 flex justify-between items-center rounded-t-3xl">
                <div>
                  ...
                  <p className="text-white/90 mt-1">
                    Resi: {selectedShipment.nomorResi || "Belum tersedia"}
                  </p>
                </div>
                <button
                  onClick={() => setShowDetailModal(false)}
                    ...
                    <div className="bg-gray-50 rounded-2xl p-6 space-y-4">
                      <div className="flex justify-between">
                        ...
                        <div
                          className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl ${
                            mapStatusToConfig(selectedShipment.status).color
                          }`}
                        >
                          {(() => {
                            const Icon = mapStatusToConfig(
                              selectedShipment.status
                            ).icon;
                            return <Icon className="w-5 h-5" />;
                          })()}
                          <span className="font-bold">
                            {mapStatusToConfig(selectedShipment.status).label}
                          </span>
                        </div>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold">
                          {selectedShipment.kurir || "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Layanan</span>
                        <span className="font-bold">
                          {selectedShipment.layanan || "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Biaya</span>
                        <span className="font-bold text-[#cb5094]">
                          {formatPrice(selectedShipment.biaya)}
                        </span>
                      </div>
                      {selectedShipment.estimasiPengiriman && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {formatDate(selectedShipment.estimasiPengiriman)}
                          </span>
                        </div>
                      )}
                      {selectedShipment.dikirimPada && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-[#cb5094]">
                            {formatDate(selectedShipment.dikirimPada)}
                          </span>
                        </div>
                      )}
                      {selectedShipment.diterimaPada && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-green-600">
                            {formatDate(selectedShipment.diterimaPada)}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 rounded-2xl p-6 space-y-3">
                      <p>
                        ...
                        <span className="font-bold">
                          {selectedShipment.order?.nomorOrder}
                        </span>
                      </p>
                      <p>
                        <span className="font-medium">Customer:</span>{" "}
                        <span className="font-bold">
                          {selectedShipment.order?.namaPenerima}
                        </span>
                      </p>
                      <p>
