...
function AdminTransactions() {
    ...
    try {
      ...
      const transactionsPromises = orders.map(async (order) => {
        try {
          ...
          const activePayment =
            sortedPayments.find((p) => p.status === "SETTLEMENT") ||
            sortedPayments[0];
            ...
            const processedItems = (order.items || []).map((item) => ({
              ...
              gambarUrl:
                ...
                (item.product?.gambarUrl
                  ? item.product.gambarUrl.split("|||")[0]
                  : null) ||
  ...
  const formatDateRange = () => {
    ...
    const endStr = dateRange.end
      ? new Date(dateRange.end).toLocaleDateString("id-ID", {
          day: "numeric",
          month: "short",
          year: "numeric",
        })
  ...
  const filteredTransactions = transactions.filter((trans) => {
    ...
    if (searchQuery) {
      ...
      if (
        !trans.transactionId?.toLowerCase().includes(searchLower) &&
  ...
  const totalRevenue = filteredTransactions
    .filter((t) =>
      ["SETTLEMENT", "SETTLED", "CAPTURE"].includes(t.status?.toUpperCase())
    )
  ...
  const handleRefund = async () => {
    ...
    setSubmitting(true);
    const loadingToast = toast.loading("Memproses refund...");
    try {
      await api.post(`/payments/${selectedTransaction.id}/refund`, {
        reason: refundReason,
      });
      ...
      toast.dismiss(loadingToast);
      toast.error(err.response?.data?.message || "Gagal proses refund");
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm lg:text-base">
                  Pantau dan kelola pembayaran pelanggan
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100">
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-gray-600">
                  ...
                  dari <span className="font-bold">{transactions.length}</span>{" "}
                  transaksi
                </p>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-pink-100 p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {hasActiveFilters()
                ...
                : "Transaksi akan muncul saat ada pembayaran dari pelanggan"}
            </p>
            {hasActiveFilters() && (
              <button
                onClick={resetFilters}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mb-2">
                        {formatDate(trans.dibuatPada || trans.createdAt)}
                      </p>
                      <p className="text-sm text-gray-600 truncate">
                        <span className="font-semibold">
                          {trans.order?.namaPenerima}
                        </span>
                      </p>
                    </div>
                    <div
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-white text-xs ${statusConfig.color} flex-shrink-0 ml-2`}
                    >
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-2xl font-bold text-[#cb5094]">
                      {formatPrice(trans.jumlah)}
                    </p>
                  </div>
            ...
            >
              ...
              <div className="p-8 space-y-8">
                ...
                <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6 bg-gray-50 rounded-2xl p-6">
                    <div>
                      ...
                      <div
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-white ${
                          mapStatusToConfig(selectedTransaction.status).color
                        }`}
                      >
                        {(() => {
                        ...
                        <span className="font-bold">
                          {mapStatusToConfig(selectedTransaction.status).label}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {(() => {
                        const methodConfig = mapMethodToConfig(
                    ...
                    <div>
                      ...
                      <p className="text-3xl font-bold text-[#cb5094]">
                        {formatPrice(selectedTransaction.jumlah)}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {formatDate(
                          selectedTransaction.dibuatPada ||
                    ...
                    {selectedTransaction.waktuSettlement && (
                      <div>
                        ...
                        <p className="font-medium text-green-600">
                          {formatDate(selectedTransaction.waktuSettlement)}
                        </p>
                      </div>
                    )}
                  ...
                  <div className="bg-gray-50 rounded-2xl p-6 space-y-3">
                    <div className="grid md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-bold">
                          {selectedTransaction.order?.nomorOrder}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-600 text-sm">Nama Penerima</p>
                        <p className="font-bold">
                          {selectedTransaction.order?.namaPenerima}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-600 text-sm">Telepon</p>
                        <p className="font-medium">
                          {selectedTransaction.order?.teleponPenerima}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-600 text-sm">Email</p>
                        <p className="font-medium">
                          {selectedTransaction.order?.user?.email || "-"}
                        </p>
                      </div>
                    </div>
                    ...
                    {selectedTransaction.order?.alamatBaris1 && (
                      <div className="pt-3 border-t border-gray-200">
                        ...
                        <p className="font-medium">
                          {selectedTransaction.order.alamatBaris1}
                          {selectedTransaction.order.alamatBaris2 &&
                            `, ${selectedTransaction.order.alamatBaris2}`}
                          <br />
                          {selectedTransaction.order.kota},{" "}
                          {selectedTransaction.order.provinsi}{" "}
                          {selectedTransaction.order.kodePos}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {selectedTransaction.order?.items &&
                  selectedTransaction.order.items.length > 0 && (
                    <div>
                      <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                        <Package className="w-5 h-5 text-[#cb5094]" />
                        Produk yang Dipesan (
                        {selectedTransaction.order.items.length} item)
                      </h3>
                      <div className="space-y-4">
                        {selectedTransaction.order.items.map((item, idx) => (
                          <div
                            key={idx}
                            className="flex gap-4 bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-all"
                          >
                            <img
                              src={item.gambarUrl}
                              alt={item.namaProduk}
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center justify-between mt-2">
                                ...
                                <p className="font-bold text-[#cb5094]">
                        ...
                        <div className="bg-[#cb5094]/10 rounded-xl p-5 space-y-2 border-2 border-[#cb5094]/20">
                          <div className="flex justify-between text-gray-700">
                            <span>Subtotal</span>
                            <span className="font-semibold">
                              {formatPrice(selectedTransaction.order.subtotal)}
                            </span>
                          </div>
                          <div className="flex justify-between text-gray-700">
                            ...
                            <span className="font-semibold">
                              {formatPrice(
                                selectedTransaction.order.ongkosKirim
                              )}
                            </span>
                          </div>
