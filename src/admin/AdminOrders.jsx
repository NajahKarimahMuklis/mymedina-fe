...
function AdminOrders() {
  ...
  const loadOrders = async () => {
    try {
      ...
      const enrichedOrders = await Promise.all(
        ordersData.map(async (order) => {
          try {
            ...
            return {
              ...order,
              shipment: shipmentRes.data.shipment || shipmentRes.data,
            };
          } catch (err) {
      ...
      const sortedOrders = enrichedOrders.sort(
        (a, b) =>
          new Date(b.dibuatPada || b.createdAt) -
          new Date(a.dibuatPada || a.createdAt)
      );
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    if (searchQuery) {
      ...
      if (
        !order.nomorOrder?.toLowerCase().includes(query) &&
  ...
  const openUpdateShipment = async (order) => {
    try {
      ...
      if (!shipment) {
        toast.error("Pengiriman belum dibuat untuk order ini");
        return;
      }

      setSelectedShipment(shipment);
      setUpdateForm({
        status: ["PENDING", "READY_TO_SHIP"].includes(shipment.status)
          ? "SHIPPED"
          : "DELIVERED",
      ...
      if (err.response?.status === 404) {
        toast.error(
          'Pengiriman belum dibuat. Klik tombol "Kirim" untuk membuat.'
        );
      } else {
  ...
  const handleUpdateShipment = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    const loadingToast = toast.loading("Mengupdate status pengiriman...");
    try {
  ...
  const openUpdateOrderStatus = (order) => {
    setSelectedOrder(order);
    setUpdateOrderStatusForm({ status: order.status });
    setShowUpdateOrderStatus(true);
  };

  const handleUpdateOrderStatus = async (e) => {
    e.preventDefault();
    if (updateOrderStatusForm.status === selectedOrder.status) {
      toast.error("Status baru sama dengan status saat ini");
      return;
    }

    setSubmitting(true);
    const loadingToast = toast.loading("Mengubah status pesanan...");
    try {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-pink-50 pb-20 lg:pb-0">
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div
                        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium ${statusConfig.color}`}
                      >
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2 bg-[#cb5094] text-white rounded-xl font-medium flex items-center justify-center gap-2"
                      >
                      ...
                      {(order.status === "PROCESSING" ||
                        ...
                        order.shipment && (
                          <button
                            onClick={() => openUpdateShipment(order)}
                            className="flex-1 py-2 bg-purple-500 text-white rounded-xl font-medium flex items-center justify-center gap-2"
                          >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-center">
                            <span
                              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${statusConfig.color}`}
                            >
                              <StatusIcon className="w-4 h-4" />
                              {statusConfig.label}
                            </span>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-center gap-3">
                              ...
                              <button
                                onClick={() => openUpdateOrderStatus(order)}
              ...
              <div className="sticky top-0 bg-gradient-to-r from-[#cb5094] to-[#e570b3] text-white px-8 py-6 flex justify-between items-center rounded-t-3xl">
                <div>
                  <h2 className="text-2xl font-bold">
                    Detail Pesanan #{selectedOrder.nomorOrder}
                  </h2>
                  <p className="text-white/90 mt-1">
                    {formatDate(selectedOrder.dibuatPada)}
                  </p>
                </div>
                <button
                  onClick={() => setShowDetailModal(false)}
              ...
              <div className="p-8 space-y-8">
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-6 border-2 border-gray-200">
                    ...
                    <div
                      className={`inline-flex items-center gap-3 px-6 py-4 rounded-2xl ${
                        mapStatusToConfig(selectedOrder.status).color
                      } shadow-lg`}
                    >
                      {(() => {
                        const StatusIcon = mapStatusToConfig(
                          selectedOrder.status
                        ).icon;
                        return <StatusIcon className="w-6 h-6" />;
                      })()}
                      <span className="text-lg font-bold">
                        {mapStatusToConfig(selectedOrder.status).label}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-[#fef5fb] to-white rounded-2xl p-6 border-2 border-[#cb5094]/20">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-lg">
                        <span>Ongkir</span>
                        <span className="font-bold">
                          {formatPrice(selectedOrder.ongkosKirim)}
                        </span>
                      </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 shadow-md">
                    ...
                    <p className="text-gray-700">
                      ...
                      {selectedOrder.alamatBaris2 &&
                        `, ${selectedOrder.alamatBaris2}`}
                      <br />
                      {selectedOrder.kota}, {selectedOrder.provinsi}{" "}
                      {selectedOrder.kodePos}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="font-bold text-gray-900 mb-6 text-xl">
                    Daftar Produk ({selectedOrder.items?.length || 0} item)
                  </h3>
                  <div className="space-y-4">
                    {(selectedOrder.items || []).map((item, idx) => (
                      <div
                        key={idx}
                        className="bg-gray-50 rounded-2xl p-5 flex gap-5"
                      >
                        <div className="w-24 h-24 bg-gray-200 rounded-xl flex items-center justify-center text-gray-500 text-4xl font-bold">
                          {item.namaProduct?.charAt(0).toUpperCase() || "P"}
                        </div>
                        <div className="flex-1">
                          <h4 className="font-bold text-gray-900 mb-1">
                            {item.namaProduct || "Produk Tidak Diketahui"}
                          </h4>
                          <p className="text-sm text-gray-600 mb-2">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">
                              {item.kuantitas || 1} ×{" "}
                              {formatPrice(item.hargaSatuan || 0)}
                            </span>
                            <span className="font-bold text-lg text-[#cb5094]">
        ...
        {showUpdateOrderStatus && selectedOrder && (
          <div
            className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
            onClick={() => setShowUpdateOrderStatus(false)}
          >
            <div
              className="bg-white rounded-3xl max-w-md w-full shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="bg-gradient-to-r from-[#cb5094] to-[#e570b3] text-white px-6 py-4 rounded-t-3xl">
                <h3 className="text-xl font-bold">Ubah Status Pesanan</h3>
                <p className="text-sm opacity-90 mt-1">
                  #{selectedOrder.nomorOrder}
                </p>
              </div>
              ...
              >
                <div>
                  ...
                  <select
                    value={updateOrderStatusForm.status}
                    onChange={(e) =>
                      setUpdateOrderStatusForm({ status: e.target.value })
                    }
