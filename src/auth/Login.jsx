...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Login berhasil
        // Simpan token & user ke localStorage (atau context jika pakai state management)
        localStorage.setItem('accessToken', data.accessToken);
        localStorage.setItem('user', JSON.stringify(data.user));

        alert('Login berhasil! Selamat datang kembali');
        // Redirect ke dashboard atau home
        window.location.href = '/dashboard'; // atau '/' jika ingin ke home
      } else {
        // Error dari backend
        if (data.message) {
          alert(data.message);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="space-y-1">
              ...
              <a href="/signup" className="block py-3 px-4 text-[#cb5094] hover:bg-[#fffbf8] rounded-lg font-bold text-sm transition-all">
                SIGN UP
              </a>
            </div>
          </div>
