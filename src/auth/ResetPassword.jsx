...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token || token === "reset-password") {
      showNotification(
        "error",
        "Token tidak ditemukan di URL. Silakan gunakan link dari email"
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showNotification(
          "error",
          data.message ||
            "Token tidak valid atau sudah kadaluarsa. Silakan minta link baru"
        );
      }
    } catch (err) {
      ...
      showNotification(
        "error",
        "Gagal terhubung ke server. Pastikan backend berjalan di " + BACKEND_URL
      );
    } finally {
      setIsLoading(false);
    }
